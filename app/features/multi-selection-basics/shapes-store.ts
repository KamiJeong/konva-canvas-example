...
type ShapeStoreState = {
  ...
  selectionBox: { visible: boolean; x: number; y: number; width: number; height: number };
};

type ShapeStoreAction = {
  selectBlank: () => void;
  selectShapes: (ids: string[]) => void;
  selectSingleShape: (id: string) => void;
  ...
  updateSelectionBox: (nextSelectionBox: Partial<ShapeStoreState["selectionBox"]>) => void;
};
...
export const useShapesStore = create<ShapeStoreState & ShapeStoreAction>((set, get) => ({
  ...
  selectBlank: () => set(() => ({ selectedIds: [] })),
  selectShapes: (ids) => set(() => ({ selectedIds: ids })),
  selectSingleShape: (id) => set(() => ({ selectedIds: [id] })),
  ...
  addShape: (newShape) => {
    set((prev) => ({ shapes: [...prev.shapes, newShape], selectedIds: [newShape.id] }));
  },
  createRect: () => {
  ...
  updateSelectionBox: (nextSelectionBox) =>
    set((prev) => ({
      selectionBox: { ...prev.selectionBox, ...nextSelectionBox },
    })),
}));
